...
class PlayerService : MediaSessionService() {
    ...
    fun applyAiEnhancer(enabled: Boolean) {
        try {
            ...
            // Real AI Enhancer detail gain boost (+500 mB)
            setEnhancerTargetGain(if (enabled) 500 else 0)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }
    ...
    fun applyDialogueBooster(enabled: Boolean) {
        val player = mediaSession?.player ?: return
        if (enabled) {
            // Apply Vocal/Dialogue track preference & gain profile
            val currentParams = player.trackSelectionParameters
            player.trackSelectionParameters = currentParams.buildUpon()
                .setPreferredAudioLanguage(playerPreferences.preferredAudioLanguage.ifEmpty { "en" })
                .build()
        }
    }
