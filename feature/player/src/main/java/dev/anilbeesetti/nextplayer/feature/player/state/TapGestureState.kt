...
class TapGestureState(
    ...
    var activeSpeed by mutableStateOf(longPressSpeed)
    var isSpeedLocked by mutableStateOf(false)

    private var resetJob: Job? = null
    private var currentSpeed: Float = player.playbackParameters.speed
    val speedPresets = listOf(1.25f, 1.5f, 2.0f, 2.5f, 3.0f)
    ...
    fun handleOnLongPressRelease() {
        if (isLongPressGestureInAction && !isSpeedLocked) {
            isLongPressGestureInAction = false
            player.setPlaybackSpeed(currentSpeed)
        }
    }
